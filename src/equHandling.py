...
def updateCobraEqu(cobra: CobraInitializer, p2: Phase2Vars, xNew):
    ...
    with margin currentEps in the equalities. If S is non-empty, take the point from S
    with best objective as ``cobra.sac_res['xbest']``. If S is empty (there are no feasible points): If the new
    point has lower maxViol than the maxViol of ``cobra.sac_res['xbest']``, take it as the new ``cobra.sac_res['xbest']``.
    ...
    s_res = cobra.sac_res
    equ_index = np.flatnonzero(s_res['is_equ'] == True)
    ...
    temp = np.array([temp, -temp[:, equ_index]])
    equ2Index = np.concat(equ_index, s_res['nConstraints'] + np.arange(0, equ_index.size))
    ...
    if currentFeas.size == 0:
        ibest = np.flatnonzero(currentMaxViols == min(currentMaxViols))[0]
    ...
    fminInd = np.flatnonzero(s_res['Fres'][currentFeas] == min(s_res['Fres'][currentFeas]))
...
def modifyMu(Cfeas, Cinfeas, Tfeas, currentEps, cobra: CobraInitializer, p2: Phase2Vars):
    """
    Modify equality margin :math:`\mu`.
    ...
    switcher = {
        ...
        'funcDim': (s_res['muVec'][0] * (1 / s_opts.EQU.dec) ** ((p2.num - 3 * s_res['A'].size[1]) /
                                                                 ((Tfeas ** 2) / 2 - 1))
                    ) + s_opts.EQU.equEpsFinal,
        'funcSDim': (s_res['muVec'][0] * (1 / s_opts.EQU.dec) ** ((p2.num - 3 * s_res['A'].size[1]) / Tfeas)
                     ) + s_opts.EQU.equEpsFinal,
        'Zhang': max(s_res['muVec'] * (1 - (s_res['currentFeas'].size / p2.num)), s_opts.EQU.equEpsFinal),
