...
class RBFoptions:
    """
        ...
        :param interpolator: "scipy" or "sacobra", which interpolation method to use. In case of "scipy", use
          ...
          SACOBRA's own implementation of RBF models (allows with ``degree=1.5`` the option equivalent to
          ``squares=T`` in SACOBRA R, which means only pure squares in the polynomial tail).
